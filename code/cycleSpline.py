...
# The goal is to read in a chunk of audio samples (say 2048 samples) and some float endpoints,
# a and b, with 0 <= a < b <= 2047, and to graph a cubic spline that approximates the audio
# graph over the interval [a,b].  The cubic spline should interpolate for n target values
# which are data points on the piecewise linear graph which connects sample points.  
...
def plotCycleSpline(waveform, cycle_num, a, b, n) :
    ...
    print("number of samples for audio signal graph:  ", count)
...
n = 24
cycle_num = 0
fig = plotCycleSpline(waveform, cycle_num, a, b, n)
