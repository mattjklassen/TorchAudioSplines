...
def genPolyWaveform(f01, f02, time, sample_rate, bcoeffs) :

    poly_cycle_length = float(1.0/100) # in seconds
    ...
    # first write 2 f01 cycles into poly_cycle
    num_cycles = 2 
    ...
    # next write 3 f02 cycles into poly_cycle
    num_cycles = 3 
    ...
    for i in range(poly_cycle_samples) :
        poly_cycle[i] = (temp1[i] + temp2[i]) / 2
...
wav_data = genPolyWaveform(f01, f02, time, sample_rate, bcoeffs)
