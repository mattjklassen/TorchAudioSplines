...
if args > 1 :
    override_params = 1
# first need to parse config.txt
...
print("total_time:  ", total_time, "   time value of melody in seconds")
...
# see recalculation of time0 if total_time > 0 below
...
# if total_time > 0 we recompute time0 to scale according to total_time
# this is simpler, since the time axis of the melodic contour is initially [0,1]
# so total time just scales [0,1] and so it scales each note, including the first with time0
# using total_time is makes more sense for melodies which are based on stationary points
# since it is unpredictable what the first note duration will be.
# using time0 can be useful for melodies which need to have some fixed note length and number of notes
# if there is any other reason to fix the length of the first note.

if total_time > 0 :
    duration_scalar = total_time
    time0 = length0 * total_time
...
print("copy of config file config.txt :\n\n")
