# ----- Brief Description -----
#
# Plot multiple cubic splines with bcoeffs coming from files listed in arg1 on command line
# and also use possible options as for single plotBcoeffs.py ...
...
# The next functions are imported to compute using the deBoor algorithm.  The first one computes
...
# value for B-spline function given all n=k+d bcoeffs.  The third has a slightly different calling 
# sequence ...
...
# these two, stat and hold, have become default settings

# we need to parse bcoeffs_file line by line to get each set of bcoeffs from other files
...
with open(bcoeffs_file, 'r')as f:
    bcoeffs_list = f.readlines()
    f.close()

for file in bcoeffs_list :
    plotBcoeffs(file)


# Here we need a function that creates the pdf and adds to a larger pdf each of the plots in the
# list, like it is done in findCycles.py
# In this pdf we should have graphs of each cycle spline and also the melodic contour spline,
# which typically also one of those cycle splines, graphed with sample and hold to indicate melody.
# This information is sufficient to create different versions of the melody, with parameters still
# to be chosen, like duration of first note, overlaps of notes, y axis scale for pitch range,
# and gain factors for each key cycle spline.  
