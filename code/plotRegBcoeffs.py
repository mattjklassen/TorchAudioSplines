...
# number of notes K as arg2 on command line, so that we have K subintervals and
# graph is sample and hold on those subintervals.  If more command line args, then use
# optional knot sequence from file as arg3, and optional inputs from file as arg4.
...
stat = 0   # if stat = 1 will plot stationary points where numerical derivative = 0
...
if stat == 0 :
    ...
    if hold == 1:
        for i in range(0, K) :
            holdxvals = [inputVals[i], inputVals[i+1]]
            holdyvals = [outputVals[i], outputVals[i]]
            plt.plot(holdxvals, holdyvals, 'blue')
